...
static void clock_setup(enum clock_mode clock)
{
  ...
  rng_enable();
#elif defined(STM32F767ZI)
  // TODO: need to work out clock setup
  switch (clock) {
  case CLOCK_BENCHMARK:
    /* Benchmark straight from the HSI16 without prescaling */
    rcc_osc_on(RCC_HSI16);
...
#if defined(DISCOVERY_BOARD) || defined(NUCLEO_BOARD) || defined(CW_BOARD) ||  defined(STM32F767ZI)
...
void hal_send_char(char in) {
  usart_send_blocking(SERIAL_USART, in);
}
